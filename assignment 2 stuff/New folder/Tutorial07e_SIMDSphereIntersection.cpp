...
bool areSpheresIntersected(const Spheres* sphereList, const Ray* r, float* t)
{
	...
	//TODO: declare an initialise a SIMD vector full of the current minimum intersection found (*t)
	...
	for (unsigned int i = 0; i < NUM_SPHERES / 8; ++i)
	{
		...
		for (unsigned int j = 0; j < 8; ++j)
		{
			...
			if (_mm256_castps_si256(Dge0).m256i_i32[j])
			{
				...
				if (_mm256_castps_si256(t0gtEPSILON).m256i_i32[j] && (t0 < *t))			// use the SIMD vesion of minimum intersection distance
				{
					*t = t0;															// use the SIMD vesion of minimum intersection distance
					totalImprovedIntersections++;
					result = true;
				}
				else if (_mm256_castps_si256(t1gtEPSILON).m256i_i32[j] && (t1 < *t))	// use the SIMD vesion of minimum intersection distance
				{
					*t = t1;															// use the SIMD vesion of minimum intersection distance
					totalImprovedIntersections++;
					result = true;
				}
			}
	...
	//TODO: write a loop over all the elements in the minimum intersection distance SIMD vector
	//TODO: find the smallest value in the SIMD vector

	//TODO: write the smallest value to *t
