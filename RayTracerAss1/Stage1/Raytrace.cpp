...
void render(Scene* scene, unsigned int threadID, int threadTotal, const int width, const int height, /*unsigned int buffer,*/ const int aaLevel, unsigned int* out)
{
	//dividing image for threads
	int threadnum = threadID;
	int heightStart = height / 2;
	int starty = height / threadTotal;
	int remainder = height % threadTotal; //intended for dealing with uneven number of threads
	int thready = starty*(threadnum+1); //determines the start of the next thread
	...
	//starting from the start of the current thread looping through to the start of the next thread
	for (int y = -heightStart + (starty * threadnum); y < -heightStart + thready; ++y)
	{
...
DWORD __stdcall renderThreadStart(LPVOID threadData)
{
	...
	render(&data->scene, data->threadID,data->threadTotal, data->width, data->height, /*data->buffer,*/ data->samples, data->out);
...
int main(int argc, char* argv[])
{
	...
	// rendering options
	int times = 1;
	unsigned int threads = 2;			// currently unused
	...
	for (int i = 0; i < times; i++)
	{
		...
		for (unsigned int i = 0; i < threads; i++) {
			...
			threadData[i].out = buffer + i * width * height / threads;//getting size of the output
