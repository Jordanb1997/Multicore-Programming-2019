...
void render(Scene* scene, unsigned int threadID, int threadTotal, const int width, const int height, /*unsigned int buffer,*/ const int aaLevel, unsigned int* out, unsigned int* lineCount)
{
	//holding thread ID as an int
	int threadnum = threadID;

	//dividing image for threads by height
	int heightStart = height / 2; //start of image
	int starty = height / threadTotal;//start of threads
	int thready = starty * (threadnum + 1);//adding space to next thread
	int newHeight = heightStart + thready;
	//dividing image for threads by width
	int widthStart = width / 2; //start of image
	int startx = width / threadTotal; //start of threads
	int threadx = startx * (threadnum + 1);//adding space to next thread
	int newWidth = widthStart + threadx;//keeping it all in one variable, for the new lines max width
	...
	//getting value from memory for linecount
	int y;
	
	// loop through all the pixels
	//using interlocked increment to interleave the threads
	while ((y = InterlockedIncrement(lineCount))< newHeight)
	{
		//dealing with looping through the current lines pixels on the x axis
		for(int x = -widthStart + (startx * threadnum); x < newWidth; ++x)
		{
...
DWORD __stdcall renderThreadStart(LPVOID threadData)
{
	...
	render(&data->scene, data->threadID,data->threadTotal, data->width, data->height, data->samples, data->out, data->lineCount);
