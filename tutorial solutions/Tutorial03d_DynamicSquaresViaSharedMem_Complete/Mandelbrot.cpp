...
void mandelbrot(unsigned int threadID, unsigned int iterations, float centrex, float centrey, float scaley, 
	unsigned int samples, unsigned int width, unsigned int height, unsigned int blockSize, unsigned int* out, unsigned int* blockCount)
{
	...
	// current block being rendered by this thread
	unsigned int blockNum;

	// potentially loop through entire image size (ie. (0,0) to (width - 1, height - 1)), on a block-by-block basis
	while ((blockNum = InterlockedIncrement(blockCount)) < widthInBlocks * heightInBlocks)
	{
		unsigned int blockx = blockNum % widthInBlocks;
		unsigned int blocky = blockNum / widthInBlocks;

		for (unsigned int iy = blocky * blockSize; iy < (blocky + 1) * blockSize; iy += 1)
		{
			for (unsigned int ix = blockx * blockSize; ix < (blockx + 1) * blockSize; ix += 1)
			{
				int totalCalc = 0;
