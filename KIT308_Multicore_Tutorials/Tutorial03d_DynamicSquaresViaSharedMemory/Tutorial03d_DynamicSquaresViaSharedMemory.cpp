// Tutorial03d_DynamicSquaresViaSharedMemory.cpp 
...
DWORD __stdcall mandelbrotThreadStart(LPVOID threadData)
{
	...
	mandelbrot(data->threadID, data->iterations, data->centrex, data->centrey, data->scaley, data->samples, data->width, data->height, data->blockSize, data->out, data->blockCount);

	ExitThread(NULL);
}

static void generate(unsigned int threads, unsigned int blockSize, unsigned int iterations, float centrex, float centrey, float scaley,
	unsigned int samples, unsigned int width, unsigned int height, unsigned int* out)
{
	...
	// index of last block generated (starts at -1 because of InterlockedIncrement)
...
int main(int argc, char **argv)
{
	...
	generate(threads, blockSize, iterations, centrex, centrey, scaley, samples, width, height, buffer);
