...
bool areSpheresIntersected(const Spheres* sphereList, const Ray* r, float* t)
{
	...
	//: declare an initialise a SIMD vector full of EPSILON
	...
	for (unsigned int i = 0; i < NUM_SPHERES / 8; ++i)
	{
		...
		//: declare and initialise a SIMD variable to store whether (D >= 0.0f)

		//: declare and initialise a SIMD variable to store whether (t0 > EPSILON)
		//: declare and initialise a SIMD variable to store whether (t1 > EPSILON)
		...
		for (unsigned int j = 0; j < 8; ++j)
		{
			...
			if (_mm256_castps_si256(Dge0).m256i_i32[j])									//: access the value from the new SIMD variable version of this test
			{
				...
				if (_mm256_castps_si256(t0gtEPSILON).m256i_i32[j] && (t0 < *t))			//: access the value from the new SIMD variable version of this first test
				{
					*t = t0;
					totalImprovedIntersections++;
					result = true;
				}
				else if (_mm256_castps_si256(t1gtEPSILON).m256i_i32[j] && (t1 < *t))		//: access the value from the new SIMD variable version of this first test
				{
					*t = t1;
					totalImprovedIntersections++;
					result = true;
