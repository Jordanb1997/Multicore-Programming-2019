...
DWORD __stdcall mandelbrotThreadStart(LPVOID threadData)
{
	...
	mandelbrot(data->threadID, data->total_threads, data->iterations, data->centrex, data->centrey, data->scaley, data->samples, data->width, data->height, data->out);
	...
	unsigned int samples, unsigned int width, unsigned int height, unsigned int* out)
{
	...
	for (unsigned int i = 0; i < threads; i++) {
		...
		threadData[i].total_threads = threads; 
